...
def load_model():
    model = tf.keras.models.load_model("Gender-Recognition Model")
    return model
...
def url_uploader():
    ...
    if path is not None:
        ...
        st.image(image, caption="Classifying the Gender", use_column_width=True)
...
def file_Uploader():
    ...
    if not file:
        show_file.info("Upload a picture of a person you want to predict.")
        return
